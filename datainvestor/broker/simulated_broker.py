...
from datainvestor.broker.broker import Broker
...
class SimulatedBroker(Broker):
    ...
    Una classe per gestire la simulazione di un broker che fornisce
    le impostazioni predefinite sia per la valuta (USD) che per la
    gestione dei costi di transazione per l'esecuzione.

    Il modello di commissione/fee predefinito è ZeroFeeModel che
    non addebita commissioni o tasse (come l'imposta di bollo).

    Parameters
    ----------
    start_dt : `pd.Timestamp`
         La data e l'ora di inizio dell'account
    exchange : `Exchange`
        Usato per determinare se l'exchange simulato è aperta, al fine
        di determinare se gli ordini possono essere eseguiti.
    data_handler : `DataHandler`
        Il gestore dati utilizzato per ottenere gli ultimi prezzi degli asset.
    account_id : `str`, optional
         L'ID account per il conto del broker.
    base_currency : `str`, optional
        La valuta con cui è denominato il conto del broker.
    initial_funds : `float`, optional
        Importo iniziale di contanti da aggiungere al conto del broker.
    fee_model : `FeeModel`, optional
        Il modello commissione/fee utilizzato per simulare commissioni/tasse.
        Il valore predefinito è ZeroFeeModel.
    slippage_model : `SlippageModel`, optional
        Il modello utilizzato per simulare lo slippage del trade.
    market_impact_model : `MarketImpactModel`, optional
        Il modello utilizzato per simulare l'impatto sul mercato del trading.
    """
    ...
    def _set_base_currency(self, base_currency):
        ...
        currency is currently not supported by datainvestor.
        ...
        if base_currency not in settings.SUPPORTED['CURRENCIES']:
            raise ValueError(
                "Currency '%s' is not supported by datainvestor. Could not "
